...
The test suite can run in a benchmark mode that transparently times every `s21_sprintf` and `sprintf` call already present in the existing test cases (one sample per call, grouped by conversion specifier), then reports the average and 95th percentile latency per specifier.

| Specifier | Samples | `s21_sprintf` avg | `s21_sprintf` p95 | `sprintf` (libc) avg | `sprintf` (libc) p95 |
|---|---|---|---|---|---|
| `%c` | 19 | 1.61 µs | 16.26 µs | 0.28 µs | 0.51 µs |
| `%s` | 28 | 1.01 µs | 1.83 µs | 0.41 µs | 1.11 µs |
| `%d` / `%i` | 40 | 0.63 µs | 0.79 µs | 0.25 µs | 0.34 µs |
| `%o` | 12377 | 0.27 µs | 0.37 µs | 0.07 µs | 0.11 µs |
| `%x` / `%X` | 12379 | 0.28 µs | 0.42 µs | 0.07 µs | 0.11 µs |
| `%u` | 25 | 0.68 µs | 1.06 µs | 0.26 µs | 0.37 µs |
| `%f` | 2299 | 1765.88 µs | 613.18 µs | 1.01 µs | 2.32 µs |
| `%e` / `%E` | 443 | 10001.84 µs | 1559.59 µs | 1.09 µs | 2.95 µs |
| `%g` / `%G` | 406 | 10724.54 µs | 1759.26 µs | 1.49 µs | 3.28 µs |
| `%n` | 21 | 28.15 µs | 140.15 µs | 0.40 µs | 1.37 µs |
| `%p` | 6 | 2.10 µs | 3.95 µs | 0.44 µs | 0.66 µs |

**Takeaways**:
- For integer/string/pointer specifiers (`c`, `s`, `d`/`i`, `o`, `x`, `u`, `n`, `p`), `s21_sprintf` is roughly 2-10x slower than glibc's `sprintf`, which is expected for an unoptimized reimplementation competing against a highly tuned libc.
- For floating-point specifiers (`f`, `e`, `g`), `s21_sprintf` is three to four orders of magnitude slower. This is the direct cost of routing every float through the custom arbitrary-precision `gmp/` library to guarantee bit-exact output parity with the native implementation.
- The p95 for `f`/`e`/`g` is lower than the average because a small number of extreme-precision/extreme-magnitude test cases dominate the mean while the bulk of calls remain comparatively fast.

Measured on an unoptimized (`-O0`) Linux/glibc build; absolute numbers will vary by machine and compiler flags, but the relative gap between the two implementations is consistent across runs.

To reproduce:
```bash
cd src
make test SPECIAL=-DS21_BENCH_SPRINTF
CK_FORK=no ./build/test/test.out
```
`CK_FORK=no` is required so all `check.h` test cases share one process and the benchmark samples accumulate instead of resetting per forked test case.
...
### 4. The `sscanf` Module
This module is fully compliant with the C standard and handles all scenarios documented on [cppreference](https://en.cppreference.com/w/c/io/fscanf.html).
